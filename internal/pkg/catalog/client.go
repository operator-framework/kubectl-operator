...
type V1Client interface {
	All(ctx context.Context) (io.ReadCloser, error)
}
...
func (c *LiveClientV1) All(ctx context.Context) (io.ReadCloser, error) {
...
func (c *portForwardClientV1) All(ctx context.Context) (io.ReadCloser, error) {
	...
	if c.cc.Status.URLs == nil {
		return nil, fmt.Errorf("cluster extension %q has no URLs", c.cc.Name)
	}
	baseURL, err := url.Parse(c.cc.Status.URLs.Base)
	if err != nil {
		return nil, err
	}
	...
	if len(labels) < 2 {
		return nil, fmt.Errorf("invalid base URL %q", c.cc.Status.URLs.Base)
	}
